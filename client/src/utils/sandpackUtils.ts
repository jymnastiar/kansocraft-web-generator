export type SandpackFiles = Record<string, string | { content?: string }>;

// Scans source files to detect npm dependencies from import statements
export function detectDependencies(files: SandpackFiles): Record<string, string> {
  const deps: Record<string, string> = {};
  if (!files) return deps;

  const codeStrings: string[] = Object.values(files).map((val) =>
    typeof val === "string" ? val : val?.content || ""
  );
  const allCode = codeStrings.join("\n");
