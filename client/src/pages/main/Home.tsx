  ...
  async function handleGenerate(prompt: string) {
    ...
    if (!user) {
      toast.add({
        ...
        description: "You must be logged in to generate a project.",
        type: "error",
      });
      navigate("/login");
      return;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col relative overflow-hidden bg-[url('/bg-image.png')] bg-cover bg-center bg-no-repeat">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 pt-12 pb-16 flex flex-col items-center text-center z-10">
        ...
        {user && !loadingProjects && projects.length > 0 && (
          <div className="w-full text-left mt-4 border-t border-border/50 pt-8">
            ...
            <div className="grid grid-cols-1 gap-3 max-h-90 overflow-y-auto pr-1 custom-scrollbar">
              {projects.map((p) => (
                <div
                  key={p._id}
                  className="group flex items-center justify-between p-4 bg-card/60 hover:bg-card hover:border-primary/50 border border-border/80 rounded-xl transition-all duration-200 shadow-sm hover:shadow"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <AlertDialog>
                      <AlertDialogTrigger
                        className="p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition-all outline-none focus-visible:ring-2 focus-visible:ring-destructive"
                    ...
                    <Link
                      to={`/builder/${p._id}`}
                      title="Open Project"
                      className="p-2 rounded-lg text-muted-foreground group-hover:text-primary hover:bg-primary/10 transition-all outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    >
                      <ArrowRight size={16} />
                    </Link>
                  </div>
